...
def save_to_excel(data, target_month):
    ...
    # 处理所有身份证号相关的列，加上英文单引号，防止Excel科学计数
    for col in df.columns:
        if '身份证' in col:
            df[col] = df[col].apply(lambda x: f"'{x}" if pd.notna(x) and str(x).strip() != '' else x)
...
if __name__ == '__main__':
    print("\n==== 审稿费统计自动化工具 ====")
    review_file = input("请输入评审表文件路径（如 data/评审.xls）：").strip()
    re_review_file = input("请输入复审表文件路径（如 data/复审.xls）：").strip()
    employee_file = input("请输入在职员工表路径（如 data/在职员工.xlsx）：").strip()
    retired_file = input("请输入退休员工表路径（如 data/退休员工.xlsx）：").strip()
    month = input("请输入目标月份（1-12）：").strip()
    try:
        month = int(month)
        current_year = datetime.now().year
        target_month = f"{current_year}-{month:02d}"
    except Exception:
        print("月份输入有误，请输入1-12的数字！")
        exit(1)
    output_file = process_and_save_data(review_file, re_review_file, employee_file, retired_file, target_month)
    if output_file:
