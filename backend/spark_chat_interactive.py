...
class SparkChatBot:
    def __init__(self):
        self.SPARKAI_URL = os.getenv('SPARKAI_URL', 'wss://spark-api.xf-yun.com/v3.5/chat')
        self.SPARKAI_APP_ID = os.getenv('SPARKAI_APP_ID')
        self.SPARKAI_API_SECRET = os.getenv('SPARKAI_API_SECRET')
        self.SPARKAI_API_KEY = os.getenv('SPARKAI_API_KEY')
        self.SPARKAI_DOMAIN = os.getenv('SPARKAI_DOMAIN', 'generalv3.5')

        if not all([self.SPARKAI_APP_ID, self.SPARKAI_API_SECRET, self.SPARKAI_API_KEY]):
            raise ValueError("缺少讯飞星火 API 凭证，请配置 .env 文件")
