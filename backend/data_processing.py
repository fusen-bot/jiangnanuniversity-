...
from utils import read_excel, is_internal, match_employee_info, check_id_card_column

def process_review_data(review_file, re_review_file, employee_file, retired_file, target_month):
    # 添加排除名单
    EXCLUDED_REVIEWERS = {'朱明', '戴陵江', '李春丽','许艳超','吴正国','陈卫','倪松涛','李旸','顾正彪','戴月波','张影陆','堵国成','马鑫','傅莉莉','孙秀兰','李雷','谢云飞','程力','翟齐啸','赵伟','商飞飞','东为富','彭新建','程建敏','樊启高','柏玉香','李兆丰'}  # 在这里添加需要排除的人员名单，但是处理过程不会有任何标记
    ...
    # 检查身份证号列的数据类型
    print("=== 检查评审表身份证号列 ===")
    review_id_check = check_id_card_column(review_df)
    print(f"评审表身份证号列检查结果: {review_id_check}")
    
    print("=== 检查复审表身份证号列 ===")
    re_review_id_check = check_id_card_column(re_review_df)
    print(f"复审表身份证号列检查结果: {re_review_id_check}")
    ...
    # 在处理审稿费之前，将排除名单中的人员审稿费设为0
    combined_df.loc[combined_df['审稿人姓名'].isin(EXCLUDED_REVIEWERS), '审稿费金额'] = 0
    
     # 为排除名单中的人员名字添加标记
    combined_df['审稿人姓名'] = combined_df['审稿人姓名'].apply(
        lambda x: f"{x}（校内排除）" if x in EXCLUDED_REVIEWERS else x
    )
