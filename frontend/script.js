document.addEventListener('DOMContentLoaded', () => {
    ...
    const authorExtractForm = document.getElementById('author-extract-form');
    ...
    if (authorExtractForm) {
        authorExtractForm.addEventListener('submit', async function(e) {
            e.preventDefault();
            ...
            extractionResult.innerHTML = '<p>正在提取数据，请稍候...</p>';

            try {
                const response = await fetch('/extract_authors', {
                    method: 'POST',
                    body: formData
                });
                ...
                extractionResult.innerHTML = `<p class="error">错误: ${error.message}</p>`;
            }
        });
    }
});
...
function displayExtractionResult(data) {
    ...
    if (!data || data.length === 0) {
        extractionResult.innerHTML = '<p>未找到通信作者信息</p>';
        return;
    }
